...
use Modules\GaugeMAX\Includes\WidgetFieldGraphMAXView;

$form = new CWidgetFormView($data);

$graph = new WidgetFieldGraphMAXView($data['fields']['graph']);
$form->registerField($graph->time_period);
...
$form
    ...
    ->addFieldset(
        (new CWidgetFormFieldsetCollapsibleView(_('Advanced configuration')))
            ...
            ->addItem([
				(new CSpan(_('PRO version')))->addClass(ZBX_STYLE_RIGHT)->addStyle('font-weight: bold; padding-top: 2rem;'),
				(new CDiv([
					new CSpan(_('➕ Line, Staircase, and Bar graph types')),
					new CTag('br'),
					new CSpan(_('➕ Adjustable graph width')),
					new CTag('br'),
					new CSpan(_('➕ Graph transparency control')),
					new CTag('br'),  
					new CSpan(_('➕ Fill level adjustment')),
					new CTag('br'),
					new CSpan(_('➕ Color selection for graph in the background')),
					new CTag('br'),
					new CSpan(_('➕ Aggregation function (e.g., average)')),
					new CTag('br'),
					new CSpan(_('➕ Custom aggregation intervals')),
					new CTag('br'),
					new CSpan(_('➕ Handling of missing data')),
					new CTag('br'),
					new CSpan(_('➕ Customizable time period for data display')),
					new CTag('br'),
					new CSpan(_('➕ Min and Max Y-axis customization')),
					new CTag('br'),
				]))->addStyle('color: #7150f7; padding-top: 2rem;')
			])
			->addItem([
				new CSpan(''),
				(new CDiv([(new CSpan('Get PRO: '))->addStyle('font-weight: bold;'), new CLink('info@initmax.com', 'mailto:info@initmax.com?subject=Inquiry%20ChatGPT%20PRO%20version')])),
			])
    )
    ->includeJsFile('widget.edit.js.php')
    ->initFormJs('widget_form.init('.json_encode([
            'thresholds_colors' => ['FF465C', 'FFD54F', '0EC9AC', '524BBC', 'ED1248', 'D1E754', '2AB5FF',
                '385CC7', 'EC1594', 'BAE37D', '6AC8FF', 'EE2B29', '3CA20D', '6F4BBC', '00A1FF', 'F3601B',
                '1CAE59', '45CFDB', '894BBC', '6D6D6D']
        ], JSON_THROW_ON_ERROR).');')
    ->show();
